...
$$
r_{i}=\operatorname{Pr}\left( i \text{correctly linked with} i^* \mid s , P \text{, worst case scenario }\right) \qquad\text{(3.5.4.1)}
$$
...
$$
s_k=\begin{cases}
\frac{1}{m!}\prod_{i=k}^q{(m-i)}&\textmd{if}\quad k\le q,\\
0&\textmd{if}\quad k>q,
\end{cases}
$$
...
$$
\lambda=\sum_{j=1}^m{\sum_{i=1}^n{\frac{d(x_{ij}^',x_{ij})}{mn}}}\in [0,1],   (3.5.11)
$$
where $d(\cdot,\cdot)\in [0,1]$ is measure of distance according to the formulas (3.5.6), (3.5.7) or (3.5.10) according to the measurement scale of a given value.
...
### Example
  ...
  The manner of assessing disclosure risk and information loss owing to the application of SDC methods was demonstrated using data from a case study published on the website of International Household Survey Network (IHSN)[^1]: Statistical Disclosure Control for Microdata: A Practice Guide - Case Study Data and R Script. Use was made of part of the code from the first study of this type, in which the authors, Thijs Benschop and Matthew Welc, applied SDC measures to a set of farms using the sdcMicro package.
