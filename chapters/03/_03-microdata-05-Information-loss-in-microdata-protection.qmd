...
$$
r_{i}=\operatorname{Pr}\left( i\quad\text{correctly linked with}\quad i^* \mid s , P \text{, worst case scenario }\right) \qquad\text{(3.5.4.1)}
$$
...
$$
r_{i} = \mathrm{E} \left( \frac{1}{F_{k}} \mid f_{k} \right) = \sum\limits_{h\geq f_{k}} \frac{1}{h} \operatorname{Pr} \left(F_{k} = h \mid f_{k} \right). \qquad\text{(3.5.1)}
$$
...
$$
{\hat{p}}_{k} = \frac{f_{k}}{\sum\limits_{i:k(i)=k}^{}w_{i}} \qquad \text{(3.5.2)}
$$ 
...
\end{cases}
$$
where $q$ is the total number of key categorical variables in the data set and $q$ -- arbitrarily defined maximum size of MSU. Thus, the smaller $k$, the larger $s_k$ and higher the risk. The SUDA score for the observation is the product of scores for all MSUs contained in it. The final SUDA score is obtained by normalizing $s_k$ by $q!$.   
...
One can easily observe that in both cases also original and protected data sets are compared. This makes it impossible to assess the original risk of disclosure, which is usually the basis of any data disclosure control activities. Below we discuss in details these methods.
...
$$
\text{dc}\left(c,c^'\right)=\frac{\left|c^{''}\text{:min}\left(c,c^'\right\leq c^{''}<\text{max}\left(c,c^'\right)\right|}{\left|D(V)\right|} (3.5.4)
$$
...
$$
\text{CTBIL}(F,G;W,K)=\sum_{\{V_{ji}\cdots V_{jt}\} f\subseteq W\atop|\{V_{ji}\cdots V_{jt}\}|\leq K}\sum_{i_1\cdots i_t}|x^F_{i_1\cdots i_t}-x^G_{i_1\cdots i_t} |(3.5.5)
$$
where $x_{\text{subscripts}}^{\text{file}}$ is the entry of the contingency table of *file* at position given by *subscripts*.

Because the number of contingency tables to be considered depends on the number of variables $|W|$, the number of categories for each variable, and the dimension $K$, a normalized version of Expression (3.5.5) may be desirable. This can be obtained by dividing Expression (3.4.2.1) by the total number of cells in all considered tables.
...
$$
H\left(V|V^'=j\right)=-\sum_{i=1}^n{p\left(V=i|V^'=j\right)\text{log}p}\left(V=i|V^'=j|\right) (3.5.6)
$$

The probabilities in Expression (3.5.6) can be derived from $P_{V,V^'}$ using Bayes's formula. Finally, the entropy-based information loss measure *EBIL* is obtained by accumulating Expression (3.5.6) for all individuals $r$ in the protected dataset $G$
...
$$d({x_{ij}}^',x_{ij})=|{x_{ij}}^'-x_{ij}|/\max_{k=1,2,\ldots,n}⁡|{x_{kj}}^'-x_{kj}|,$$ (3.5.10)
or
$$d({x_{ij}}^',x_{ij})=({x_{ij}}^'-x_{ij})^2/\max_{k=1,2,\lots,n}⁡({x_{kj}}^'-x_{kj})^2,$$ (3.5.11)
...
$$
d({x_{ij}}^',x_{ij})=\frac{2}{\pi}\arctan|{x_{ij}}^'-x_{ij}|.  (3.5.12)
$$
...
$$
\lambda=\sum_{j=1}^m{\sum_{i=1}^n{\frac{d(x_{ij}^',x_{ij})}{mn}}}\in [0,1],   (3.5.13)
$$
where $d(\cdot,\cdot)\in [0,1]$ is measure of distance according to the formulas (3.5.8), (3.5.9) or (3.5.12) according to the measurement scale of a given value.
